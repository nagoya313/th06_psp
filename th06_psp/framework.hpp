...
class framework : private boost::noncopyable {
 public:
  framework()
      : application_(), graphic_(), audio_(), actor_list_(), bgm_list_(), se_list_(), lua_(lua_open(), &lua_close), lua_top_(lua_top()), log_("log.txt", std::ios::out | std::ios::app) {
    psp::input::init_controller();
    const int result = luaL_dofile(lua_.get(), "script/th06_psp.lua");
    if (result) {
      log_ << "errro: " << lua_tostring(lua_.get(), -1) << std::endl;
    }
 ...
 private:
  int lua_top() {
    ...
    glue::init(&actor_list_, bgm_list_, &se_list_, &log_);
  ...
  void render() const {
    const psp::graphic::scoped_render render;
    psp::graphic::clear_screen();
    BOOST_FOREACH(const actor_element &element, actor_list_) {
